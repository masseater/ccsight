...
pub struct Usage {
    ...
    /// Per-TTL breakdown of `cache_creation_input_tokens`. 1h tokens bill
    /// at 2x base (vs 5m at 1.25x), so missing this split undercounts cost
    /// for 1h-TTL users. Absent on old JSONL → fall back to flat 5m rate.
    #[serde(default)]
